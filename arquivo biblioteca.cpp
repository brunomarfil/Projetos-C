...
typedef 	struct	livro {
	...
	char	titulo[20];		// título do livro
...
void emprestimo(aluno *pa,livro *pl){
	...
	fflush(stdin);
	maiuscula(aux_liv);
	ind=busca_livro(pl,aux_liv);		//busca a posição do equipo disponivel
	if(ind==-1)
...
void devolucao(aluno *pa,livro *pl){
	...
	if(achou==-1)
	...
	else
	  {
		...
		printf("\nTitulo do livro: %s",pl->titulo);			//mostra título
		printf("\nDias: %i",pa->dias);
